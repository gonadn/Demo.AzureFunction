...
namespace Demo.AzureFunction
{
    public class GetSiteDocuments
    {
            ...
            ILogger log)
        {
            ...
            {
                log.LogInformation("GetSiteListItems()");
                log.LogInformation("Getting all the list items on site.");

                IList lokasjon = await pnpContext.Web.Lists.GetByTitleAsync("TestListAdn", p => p.Title,
                                                                     p => p.Fields.QueryProperties(p => p.InternalName,
                                                                                                p => p.FieldTypeKind,
                                                                                                p => p.TypeAsString,
                                                                                                p => p.Title));

                await lokasjon.LoadListDataAsStreamAsync(new RenderListDataOptions()
                {
                    RenderOptions = RenderListDataOptionsFlags.ListData
                });

                foreach (var listItem in lokasjon.Items.AsRequested())
                {
